...
# 读取 HTML 头部（到 <style> 开始之前）
HTML_HEAD=$(sed -n '1,7p' "$SRC_DIR/index.html")

# 读取 CSS 内容（从 style.css）
CSS_CONTENT=$(cat "$SRC_DIR/css/style.css")

# 读取 HTML 主体（从 </style> 到 </head> 和 body 内容）
# 先读取到 script 标签之前的 HTML 结构
HTML_BODY=$(sed -n '/<body>/,/<!-- 开发时：外部链接 JS -->/p' "$SRC_DIR/index.html" | head -n -1)
